...
void CMapLayers::OnInit()
{
	if(m_Type == TYPE_BACKGROUND)
	{
		...
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "maps/%s_%s.map", g_Config.m_ClMenuMap, (HourOfTheDay >= 6 && HourOfTheDay < 18) ? "day" : "night");
...
void CMapLayers::BackgroundMapUpdate()
{
	if(m_Type == TYPE_BACKGROUND && m_pMenuMap)
	{
		...
		char aBuf[128];
		str_format(aBuf, sizeof(aBuf), "maps/%s.map", g_Config.m_ClMenuMap, (HourOfTheDay >= 6 && HourOfTheDay < 18) ? "day" : "night");
		if(!m_pMenuMap->Load(aBuf, m_pClient->Storage()))
		{
			str_format(aBuf, sizeof(aBuf), "map '%s' not found", g_Config.m_ClMenuMap);
			Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "client", aBuf);
