...
void CAutoUpdate::ExecuteExit()
{
	...
	if (m_NeedUpdateClient)
	{
		...
		#else
			if (rename("teeworlds_tmp","teeworlds"))
...
void CAutoUpdate::CheckUpdates(CMenus *pMenus)
{
	...
	Reset();
	IOHANDLE updFile = io_open("bla.upd", IOFLAG_READ);
	if (!updFile)
		return;
...
void CAutoUpdate::DoUpdates(CMenus *pMenus)
{
			...
			if (aReadBuf[i]=='\n')
			{
				...
				if (m_NeedUpdate)
				{
					if (cmd == '@')
					{
						...
						{
							str_format(aBuf, sizeof(aBuf), "Updating Client to %s", last_version);
							...
							#if defined(CONF_FAMILY_WINDOWS)
							if (!GetFile("teeworlds.exe", "teeworlds_tmp.exe"))
							#elif defined(CONF_ARCH_AMD64)
							if (!GetFile("teeworlds64", "teeworlds_tmp"))
							#else
							if (!GetFile("teeworlds", "teeworlds_tmp"))
							#endif
								dbg_msg("autoupdate", "Error downloading new version");
						}
						if (!m_NeedUpdateServer && ReadData.substr(2).compare("UPDATE_SERVER") == 0)
						{
							str_format(aBuf, sizeof(aBuf), "Updating Server to %s", last_version);
							pMenus->RenderUpdating(aBuf);
	...
	if (m_Updated)
	{
		...
		str_format(aBuf, sizeof(aBuf), "Client updated successfully");
		pMenus->RenderUpdating(aBuf);
		thread_sleep(200);
	}
	else
...
bool CAutoUpdate::GetFile(const char *pFile, const char *dst)
{
	...
	//Lookup
	if(net_host_lookup(g_Config.m_ClUpdateServer, &HostAddress, NETTYPE_IPV4) != 0)
	{
